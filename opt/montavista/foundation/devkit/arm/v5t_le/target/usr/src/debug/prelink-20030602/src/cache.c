...
prelink_find_entry (const char *filename, dev_t dev, ino64_t ino, int insert)
{
  ...
  if (*devino_slot != NULL)
    {
      ...
      canon_filename = wrap_canonicalize_file_name (filename);
  ...
  ent->canon_filename = wrap_canonicalize_file_name (filename);
...
prelink_save_cache (int do_warn)
{
  ...
  fd = wrap_open (prelink_cache, O_WRONLY | O_CREAT, 0644);
  if (fd < 0)
    {
