...
static LONGEST read_offset (bfd *, char *, int, int *);
...
static int get_scope_pc_bounds (struct die_info *,
				CORE_ADDR *, CORE_ADDR *,
				struct dwarf2_cu *);
		     ...
		     char *info_ptr, bfd *abfd)
{
  ...
  info_ptr += 2;
  cu_header->abbrev_offset = read_offset (abfd, info_ptr,
	  ...
	  || pdi->tag == DW_TAG_enumeration_type)
	{
	  switch (pdi->tag)
	      ...
	      if (pdi->has_pc_info)
		{
		  if (pdi->lowpc < *lowpc)
		    {
		      /* FIXME drow/2003-09-22: Should check just this
			 objfile.  */
		      if (find_pc_sect_section (pdi->lowpc + baseaddr, NULL))
			*lowpc = pdi->lowpc;
		    }
...
read_file_scope (struct die_info *die, struct dwarf2_cu *cu)
{
  ...
  ret = get_scope_pc_bounds (die, &lowpc, &highpc, cu);
		      ...
		      CORE_ADDR *highpc, struct dwarf2_cu *cu)
{
  ...
  /* Larger sections can be discarded too, so the offset doesn't have to be
     zero.  If the function is outside of any of the object file's sections,
     consider it invalid.  */
  if (find_pc_sect_section (low + baseaddr, NULL) == NULL)
    return 0;
...
/* Get the low and high pc's represented by the scope DIE, and store
   ...
   determined, set *LOWPC to -1 and *HIGHPC to 0.  Returns 1 if
   DW_AT_ranges (discontinuous ranges) were involved in the result.  */
		     ...
		     struct dwarf2_cu *cu)
{
  ...
  ret = dwarf2_get_pc_bounds (die, &current_low, &current_high, cu);
  if (ret != 0)
    {
      ...
      while (child && child->tag)
	{
	  switch (child->tag) {
	  case DW_TAG_subprogram:
	    ret = dwarf2_get_pc_bounds (child, &current_low, &current_high, cu);
	    if (ret != 0)
	      {
	  ...
	  case DW_TAG_namespace:
	    ...
	    ret = get_scope_pc_bounds (child, &current_low, &current_high, cu);
...
/* Read an offset from the data stream.  The size of the offset is
   given by OFFSET_SIZE, taken from the section header.  */

static LONGEST
read_offset (bfd *abfd, char *buf, int offset_size, int *bytes_read)
{
  LONGEST retval = 0;
...
{
  LONGEST str_offset = read_offset (abfd, buf, cu_header->offset_size,
				    (int *) bytes_read_ptr);
			  ...
			  struct dwarf2_cu *cu)
{
  ...
  struct line_header *lh;
  char *line_ptr;
  int bytes_read, initial_bytes_read;
  int i;
  ...
  /* Read in the header.  */
  lh->total_length = read_initial_length (abfd, line_ptr, NULL,
					  &initial_bytes_read);
  line_ptr += initial_bytes_read;
  ...
  line_ptr += 2;
  lh->header_length = read_offset (abfd, line_ptr,
				   initial_bytes_read == 4 ? 4 : 8,
				   &bytes_read);
  line_ptr += bytes_read;
