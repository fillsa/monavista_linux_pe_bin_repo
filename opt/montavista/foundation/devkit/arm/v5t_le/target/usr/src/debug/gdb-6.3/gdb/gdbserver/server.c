...
handle_query (char *own_buf)
{
  ...
  if (strcmp ("qFollowParent", own_buf) == 0)
    {
      (*the_target->follow) (0);
      strcpy (own_buf, "OK");
      return;
  ...
  if (strcmp ("qFollowChild", own_buf) == 0)
    {
      (*the_target->follow) (1);
      strcpy (own_buf, "OK");
      return;
  ...
  if (strncmp ("qStartDetach:", own_buf, 13) == 0)
    {
      int pid = atoi (own_buf + 13);
      extern int linux_memory_pid;
      linux_memory_pid = pid;
      strcpy (own_buf, "OK");
      return;
    }

  if (strcmp ("qEndDetach", own_buf) == 0)
    {
      extern int linux_memory_pid;
      linux_memory_pid = 0;
      strcpy (own_buf, "OK");
  ...
  if (strncmp ("qPassSignals:", own_buf, strlen ("qPassSignals:")) == 0)
    {
      ...
      char *p = own_buf + strlen ("qPassSignals:");
      unsigned char cursig;

      unhexify ((char *) &cursig, p, 1);
      for (i = 0; i < numsigs; i++)
	{
	  if (i == cursig)
	    {
