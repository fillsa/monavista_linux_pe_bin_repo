...
struct target_ops
{
  ...
  /* Return 1 if the thread with process ID PID is alive.  */
...
int mywait (char *statusp, int connected_wait);
...
/* Some "special" results which can be returned from mywait.  */
#define FORK_STATUS (-2)
extern unsigned long pending_fork_pid;
extern unsigned long pending_fork_vfork;
extern unsigned long pending_fork_thread;

#define EXEC_STATUS (-3)
extern char pending_exec_name[256];
