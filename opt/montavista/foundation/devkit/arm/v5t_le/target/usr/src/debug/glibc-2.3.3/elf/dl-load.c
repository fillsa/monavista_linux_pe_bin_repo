...
/* convenience struct for passing around ELF numapolicy info */
struct numapolicy_info {
	int mode;
	unsigned long flags;
	int maxnode;
	int num_nodes;
...
#ifdef HAVE_NUMA_POLICY
static int __have_no_mbind;
#endif

static void *
__do_mmap(caddr_t addr, size_t len, int prot,
	  int flags, int fd, off_t offset,
	  struct numapolicy_info * policy)
{
	void * result = __mmap(addr, len, prot, flags, fd, offset);
	
#ifdef HAVE_NUMA_POLICY
	if (result != MAP_FAILED) {
		if (policy && policy->num_nodes && !__have_no_mbind) {
			int saved_errno = errno;
			int err = __mbind(result, len, policy->mode,
					  policy->nodes.n, policy->maxnode,
					  policy->flags);
			if (err) {
...
const char* map_lock (int locked_load_mode,
		      ElfW(Addr) mapstart, size_t len,
		      int prot, int fixed, int fd, off_t offset,
		      ElfW(Addr) *map_rv,
		      struct numapolicy_info * policy)
{
  caddr_t mapat;
  ...
  if (! (locked_load_mode & (RTLD_LOCK_DEPENDENT_LIB_PAGES
			     | RTLD_LOCK_LIB_PAGES)))
    {
      mapat = __do_mmap ((caddr_t) mapstart, len, prot,
			 fixed|MAP_COPY|MAP_FILE|MAP_DENYWRITE,
			 fd, offset, policy);
      if (mapat == MAP_FAILED)
    ...
    {
      mapat = __do_mmap ((caddr_t) mapstart, len, prot | PROT_WRITE,
			 fixed|MAP_ANON|MAP_PRIVATE, ANONFD, 0, policy);
      if (mapat == MAP_FAILED)
	return N_("failed to map anonymous segment");
      if (__lseek(fd, offset, SEEK_SET) != offset)
	return N_("Failed to seek file for mapping");
      if (safe_read(fd, mapat, len) == -1)
	return N_("Failed to read file into memory");
      if (__mprotect(mapat, len, prot) == -1)
	return N_("Failed to set protection for memory");
    }

  if (map_rv)
    *map_rv = (ElfW(Addr)) mapat;
  return NULL;
}

#ifdef HAVE_NUMA_POLICY
static void
make_numapolicy_info(struct numapolicy_info * pol, int mode,
		    int flags, const ElfW(Half) * nodes, int num_nodes)
{
        int i;
	for (i=0; i < num_nodes; i++) {
		int nid = nodes[i];
		if (nid >= 0) {
			nodemask_set(&pol->nodes, nid);
		} else {
			nodemask_zero(&pol->nodes);
			pol->num_nodes = 0;
			_dl_debug_printf ("no such node %d\n", nid);
			return;
		}
	}
	pol->mode = mode;
	pol->flags = flags;
	pol->num_nodes = num_nodes;
	pol->maxnode = NUMA_NUM_NODES+1;
}

static void
convert_numapolicy_note(struct numapolicy_info * textpol,
			struct numapolicy_info * datapol,
			const ElfW(numapolicy) * elfpol)
{
	const ElfW(Half) * nodelist;
	
	/* cvt the note to mem policies for text and data segments */
	nodelist = elfpol->nodes;
	make_numapolicy_info(textpol, elfpol->text_mode,
			    elfpol->text_flags, nodelist,
			    elfpol->num_text_nodes);
	nodelist += elfpol->num_text_nodes;
	make_numapolicy_info(datapol, elfpol->data_mode,
			    elfpol->data_flags, nodelist,
			    elfpol->num_data_nodes);
}

/*
 * Max size allowed for the NUMA policy note, which
 * consists of the note header, max 32 chars for the note
 * name, and the numapolicy structure including a maximum
 * of 8 listed nodes for text, data, and process.
 */
#define NUMAPOLICY_MAX_NOTE_SIZE \
    (sizeof(ElfW(Nhdr))          \
     + 32                        \
     + sizeof(ElfW(numapolicy))  \
     + 8*3*sizeof(ElfW(Half)))

static int
find_numapolicy_note(int fd, struct filebuf *fbp,
		    const ElfW(Ehdr) * eh, const ElfW(Phdr) * pht,
		    struct numapolicy_info * textpol,
		    struct numapolicy_info * datapol)
{
	int i;
	const ElfW(Phdr) * ph;
	...
	ElfW(Off) elfpol_off;
	ElfW(Nhdr) * nh;
	ElfW(Nhdr) * nh_auto = alloca (NUMAPOLICY_MAX_NOTE_SIZE);
	
	textpol->num_nodes = datapol->num_nodes = 0;
	nodemask_zero(&textpol->nodes);
	nodemask_zero(&datapol->nodes);

	for (i = 0, ph = pht; i < eh->e_phnum; i++, ph++) {
		if (ph->p_type != PT_NOTE || ph->p_align < 4 ||
		    ph->p_filesz > NUMAPOLICY_MAX_NOTE_SIZE)
			continue;
	
		if (ph->p_offset + ph->p_filesz <= (size_t) fbp->len) {
			nh = (ElfW(Nhdr) *)(fbp->buf + ph->p_offset);
		} else {
			nh = nh_auto;
			__lseek (fd, ph->p_offset, SEEK_SET);
			if ((size_t) __libc_read(fd, (void *)nh,
						 ph->p_filesz) != ph->p_filesz) {
				_dl_debug_printf ("failed to read numapolicy note\n");
				return -1;
			}
		}

		name = (const char *) ((void *)nh + sizeof(ElfW(Nhdr)));
		if (nh->n_type != NT_NUMAPOLICY ||
		    strncmp(name, "MontaVista Software", nh->n_namesz)) {
			continue;
		}

		/* the note contents start at the next 4-byte
		   boundary after the name string */
		elfpol_off = (((sizeof(ElfW(Nhdr)) + nh->n_namesz) + 3) & ~3);
		elfpol = (const ElfW(numapolicy) *)((void *)nh + elfpol_off);
		convert_numapolicy_note(textpol, datapol, elfpol);
		break;
	}

	return 0;
}
#endif /* HAVE_NUMA_POLICY */
...
_dl_map_object_from_fd (const char *name, int fd, struct filebuf *fbp,
			...
			int mode, void **stack_endp, int locked_load_mode)
{
  ...
  const char *errstring = NULL;
  int errval = 0;
  struct numapolicy_info textpol, datapol;
...
#ifdef HAVE_NUMA_POLICY
  find_numapolicy_note(fd, fbp, header, phdr, &textpol, &datapol);
#endif
...

  {
	...
	case PT_LOAD:
	  ...
	  if (ph->p_flags & PF_X)
	    c->prot |= PROT_EXEC;
#endif
	  c->policy = (ph->p_flags & PF_X) ? &textpol : &datapol;
	  break;
...
cannot allocate TLS data structures for initial thread");
	...
	errstring = map_lock (locked_load_mode, mappref, maplength, c->prot,
			      0, fd, c->mapoff, &(l->l_map_start), c->policy);
	if (__builtin_expect (errstring, 0))
	  {
	  map_error:
	    /* Error string is returned from map_lock().  */
	    goto call_lose_errno;
	  }
	...
	if (c->mapend > c->mapstart
	    ...
	    && (errstring = map_lock (locked_load_mode,
				      l->l_addr + c->mapstart,
				      c->mapend - c->mapstart,
				      c->prot, MAP_FIXED, fd, c->mapoff,
				      NULL, c->policy)))
	  goto map_error;
	...
	if (c->allocend > c->dataend)
	  {
		...
		caddr_t mapat;
		mapat = __do_mmap((caddr_t) zeropage, zeroend - zeropage,
