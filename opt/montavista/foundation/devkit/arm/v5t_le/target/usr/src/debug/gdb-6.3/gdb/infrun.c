...
follow_exec (int pid, char *execd_pathname)
{
 ...
 /* If we can't find one, things are in a very strange state...  */
  ...
  {
    extern char *solib_absolute_prefix;
    if (solib_absolute_prefix)
      {
	char *name = xmalloc (strlen (solib_absolute_prefix)
			      + strlen (execd_pathname)
			      + 1);
	strcpy (name, solib_absolute_prefix);
...
wait_for_inferior (void)
{
  ...
  while (1)
	...
	ecs->ptid = target_wait (ecs->waiton_ptid, ecs->wp);
      ...
      if (infrun_debug)
	{
	  fprintf_unfiltered (gdb_stdlog, "infrun: wait returned an event for "
			      "%s\n", target_pid_to_str (ecs->ptid));
	  if (!ptid_equal (inferior_ptid, ecs->ptid))
	    fprintf_unfiltered (gdb_stdlog, "infrun: inferior_ptid was %s\n",
...
handle_inferior_event (struct execution_control_state *ecs)
{
  ...
  if (stop_signal == TARGET_SIGNAL_TRAP)
      ...
      else if (SOFTWARE_SINGLE_STEP_P () && singlestep_breakpoints_inserted_p)
	{
	  ...
	  if (!ptid_equal (singlestep_ptid, ecs->ptid)
	      && in_thread_list (singlestep_ptid))
	    {
	      if (infrun_debug)
		fprintf_unfiltered (gdb_stdlog,
				    "Software single step trap recognized "
				    "(wrong thread)\n");
	...
	{
	  if (infrun_debug)
	    fprintf_unfiltered (gdb_stdlog, "Software single step trap "
				"recognized\n");
...
the step command stops at a different source line.", &setlist);
  ...
  deprecated_add_show_from_set (add_set_cmd ("running", class_maintenance, var_zinteger,
				  &infrun_debug, "Set program run state debugging.\n\
When non-zero, program run state internal debugging is enabled.", &setdebuglist),
		     &showdebuglist);
