...
new_thread_notify (int id)
{
  ...
  /* The `n' response is not yet part of the remote protocol, but MV uses it.  */
  if (0)
    return;
...
dead_thread_notify (int id)
{
  ...
  /* The `x' response is not yet part of the remote protocol, but MV uses it.  */
  if (0)
    return;
...
unsigned long pending_fork_thread;
unsigned long pending_fork_pid;
unsigned long pending_fork_vfork;
char pending_exec_name[256];

void
prepare_resume_reply (char *buf, char status, int signo)
{
  *buf++ = status;

  if (signo == FORK_STATUS)
    {
      *buf++ = pending_fork_vfork ? 'V' : 'R';
      sprintf (buf, "%d;", pending_fork_pid);
      buf += strlen (buf);
    }
  else if (signo == EXEC_STATUS)
    {
      *buf++ = 'X';
      hexify (buf, pending_exec_name, strlen (pending_exec_name));
      buf += strlen (buf);
      *buf++ = ';';
    }
  else
