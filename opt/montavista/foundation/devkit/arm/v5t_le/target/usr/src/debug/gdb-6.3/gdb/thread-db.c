...
check_for_thread_db (void)
{
  ...
  /* Don't attempt to use thread_db on targets which can not run
     (executables not running yet, core files).  */
...
thread_db_fetch_registers (int regno)
{
  ...
  err = td_thr_getgregs_p (&thread_info->private->th, (prgregset_t *) gregset);
...
thread_db_store_registers (int regno)
{
  ...
  err = td_thr_setgregs_p (&thread_info->private->th, (prgregset_t *) gregset);
  if (err != TD_OK)
...
thread_db_pid_to_str (ptid_t ptid)
{
  if (is_thread (ptid))
      ...
      if (ti_p == NULL)
	{
      ...
      else if (ti_p->ti_state == TD_THR_ACTIVE && ti_p->ti_lid != 0)
	{
