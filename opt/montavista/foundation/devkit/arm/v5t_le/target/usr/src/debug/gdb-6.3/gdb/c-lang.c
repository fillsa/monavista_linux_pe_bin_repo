...
static CORE_ADDR
cp_skip_trampoline (CORE_ADDR stop_pc)
{ 
  ...
  msym = lookup_minimal_symbol_by_pc (real_stop_pc);
  if (msym == NULL)
    return 0;
  ...
  new_sym = lookup_symbol (SYMBOL_DEMANGLED_NAME (msym), NULL, VAR_DOMAIN, NULL, NULL);
  if (new_sym == NULL || SYMBOL_CLASS (new_sym) != LOC_BLOCK)
    return 0;

  if (strcmp (SYMBOL_LINKAGE_NAME (new_sym), SYMBOL_LINKAGE_NAME (msym)) == 0)
    return 0;

  /* If there were two symbols with this demangled name, but different
     mangled names, then assume that the one is a trampoline for the
     other.  There's some potential for ODR problems here if we have
     the same function in an object and a shared library... so bail if
     they're in different bfd's.  */
  if (SYMBOL_BFD_SECTION (new_sym) && SYMBOL_BFD_SECTION (msym)
      && SYMBOL_BFD_SECTION (new_sym)->owner != SYMBOL_BFD_SECTION (msym)->owner)
    return 0;

  return BLOCK_START (SYMBOL_BLOCK_VALUE (new_sym));
}

...
const struct language_defn c_language_defn =
{
  ...
  cp_skip_trampoline,		/* Language specific skip_trampoline */
  NULL,				/* value_of_this */
