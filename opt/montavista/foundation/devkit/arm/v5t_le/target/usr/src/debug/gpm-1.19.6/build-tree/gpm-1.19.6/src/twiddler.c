...
{
  int fd;
  char *consolename = ret_devfs_onet(GPM_TYPE_CON);
...
int twiddler_exec(char *s)
{
  ...
  switch(pid=fork())
    ...
    case 0:
      close(0); close(1); close(2); /* very rude! */
      
      /* changes for devfs: see gpm.{c,h} for details 
       * consolename is generated in the first steps of main().
       * GPM_NULL_DEV is found in gpmInt.h */
      open(GPM_NULL_DEV,O_RDONLY);
      open(consolename,O_WRONLY);
      dup(1);
...
static inline int twiddler_use_item(char *item)
{
  ...
  /* a special function */
	   ...
	   i = (struct twiddler_active_fun *)item - twiddler_active_funs,
           i>=0 && i < active_fun_nr)
    twiddler_do_fun(i);
  ...
  if (retval)
    fprintf(stderr,"%s: ioctl(TIOCSTI): %s\n", prgname, strerror(errno));
  close(fd);
  return retval;
}
...
#define DIF_TIME(t1,t2) ((t2.tv_sec -t1.tv_sec) *1000+ \
  ...
  if ((message < last_message) && !marked) /* ok, do it */
    ...
    if (last_pressed != last_message)
      { nclick=1; GET_TIME(tv1); } /* first click (note: this is release) */
...
char *twiddler_rest_to_value(char *s)
{
  ...
  for (fptr = twiddler_functions; fptr->name; fptr++)
	...
	{
	fprintf(stderr,"%s: too many special functions (max is %i)\n",
		prgname, TWIDDLER_MAX_ACTIVE_FUNS);
	return s;
	}
...
int twiddler_key_init(void)
{
  ...
  if (!(f = fopen(TW_SYSTEM_FILE,"r")))
    {
    fprintf(stderr,"%s: " TW_SYSTEM_FILE ": %s\n",
	    prgname, strerror(errno));
    return -1;
    }
  ...
  while (files[fileindex])
	...
	{
	fprintf(stderr,"%s: " TW_SYSTEM_FILE ":%i: Syntax error\n",
		prgname, lineno);
	errcount++;
	continue;
	}
	...
	{
	fprintf(stderr,"%s: " TW_SYSTEM_FILE ":%i: Unknown modifier \"%s\"\n",
		prgname, lineno, mod);
	errcount++;
	continue;
	}
	...
	{
	fprintf(stderr,"%s: " TW_SYSTEM_FILE ":%i: "
		"Incorrect chord \"%s\"\n",
		prgname, lineno, chord);
	errcount++;
	continue;
	}
	...
	{
	fprintf(stderr,"%s: " TW_SYSTEM_FILE ":%i: "
		"Warning: Chord \"%s%s%s\" redefined\n",
		prgname, lineno, mod, mod ? " " : "", chord);
	}
      /* all done */
      if (value)
	table[index]=value;
