...
struct remote_state
{
  ...
  /* Description of the remote protocol registers indexed by REGNUM
     (making an array NUM_REGS + NUM_PSEUDO_REGS in size).  */
...
/* Should we try the 'qPassSignals' (target signal short-circuit) request? */
static struct packet_config remote_protocol_qPassSignals;

static void
set_remote_protocol_qPassSignals_packet_cmd (char *args, int from_tty,
				  struct cmd_list_element *c)
{
  update_packet_config (&remote_protocol_qPassSignals);
}

static void
show_remote_protocol_qPassSignals_packet_cmd (char *args, int from_tty,
				  struct cmd_list_element *c)
{
  show_packet_config_cmd (&remote_protocol_qPassSignals);
}
...
static void
show_remote_protocol_qPart_availableRegisters_packet_cmd (char *args, int from_tty,
					    struct cmd_list_element *c)
{
...
{
  if (remote_protocol_qPassSignals.support != PACKET_DISABLE)
    {
      ...
      for (i = 0; i < numsigs; i++)
	{
      ...
      qPass = xmalloc (count * 3 + strlen ("qPassSignals:") + 1);
      strcpy (qPass, "qPassSignals:");
      qPassTmp = qPass + strlen (qPass);
      for (i = 0; i < numsigs; i++)
	{
	  if (signal_stop_state (i) == 0
	      && signal_print_state (i) == 0
	      && signal_pass_state (i) == 1)
	    {
	      qPassTmp = pack_hex_byte (qPassTmp, i);
	      if (count)
		*qPassTmp++ = ';';
	      else
		break;
      ...
      *qPassTmp = 0;
      if (!last_qPass || strcmp (last_qPass, qPass))
	{
	  struct remote_state *rs = get_remote_state ();
	  ...
	  packet_ok (buf, &remote_protocol_qPassSignals);
	  if (last_qPass)
	    xfree (last_qPass);
	  last_qPass = qPass;
	}
...
set_thread (int th, int gen)
{
  ...
  /* While we're here, update the inferior on signals to silently pass.  */
  remote_pass_signals ();
	       ...
	       int extended_p, int async_p)
{
  ...
  /* Make sure we resync the passed signals list.  */
  if (last_qPass)
    {
      xfree (last_qPass);
      last_qPass = NULL;
...
static void
remote_start_detach (int target_pid)
{
  struct remote_state *rs = get_remote_state ();
  char *buf = alloca (rs->remote_packet_size);

  sprintf (buf, "qStartDetach:%d", target_pid);
  putpkt (buf);
  getpkt (buf, (rs->remote_packet_size), 0);
}
...
static int
remote_follow_fork (int follow_child)
{
  ptid_t last_ptid;
  ...
  if (! follow_child)
    {
      ...
      putpkt ("qFollowParent");
      getpkt (buf, (rs->remote_packet_size), 0);

      /* Now the parent is stopped and the child is freed.  If we had
         to remove breakpoints in the parent, put them back.  */
	...
	case 'T':		/* Status with PC, SP, FP, ... */
	  {
		...
		sscanf (buf+2, "%d", &status->value.related_pid);
	      }

	    /* Check for exec status.  */
	    if (buf[1] == 'X')
	      {
		int cnt;
		...
		status->value.execd_pathname = xmalloc ((p - buf) / 2);
		cnt = hex2bin (&buf[2], status->value.execd_pathname,
			       (p - buf - 3) / 2);
		status->value.execd_pathname[cnt] = 0;
	      }
...
Packet: '%s'\n",
	...
	case 'O':		/* Console output */
	  ...
	  continue;
	case 'n':               /* drow: New thread.  Informational only.  */
	  {
	    char *p_temp;
	    ULONGEST threadnum;
	    ptid_t newthread;

	    p_temp = unpack_varlen_hex (&buf[1], &threadnum);
	    newthread = pid_to_ptid (threadnum);
	    if (!in_thread_list (newthread))
	      {
		add_thread (newthread);
		ui_out_text (uiout, "[New ");
		/* FIXME - better text from threadinfo?  What else should be in this packet?  */
		ui_out_text (uiout, target_pid_to_str (newthread));
		ui_out_text (uiout, "]\n");
	      }
	  ...
	  continue;
	case 'x':               /* drow: Dead thread.  Informational only.  */
	  {
	    char *p_temp;
	    ULONGEST threadnum;
	    ptid_t deadthread;
	    ...
	    p_temp = unpack_varlen_hex (&buf[1], &threadnum);
	    deadthread = pid_to_ptid (threadnum);
	    if (in_thread_list (deadthread))
	      {
		/* FIXME Core GDB cannot deal with us deleting the
		   current thread.  */
		if (! ptid_equal (deadthread, inferior_ptid))
		  {
		    struct thread_info *tp = find_thread_pid (deadthread);
		    delete_step_resume_breakpoint_later (tp);
		    delete_thread (deadthread);
		  }

		ui_out_text (uiout, "[");
		/* FIXME - better text from threadinfo?  What else should be in this packet?  */
		ui_out_text (uiout, target_pid_to_str (deadthread));
		ui_out_text (uiout, " exited]\n");
	      }
	    /* FIXME better warning */
	    if (*p_temp != 0)
	      warning ("extra data");
...
(e.g. /dev/ttyS0, /dev/ttya, COM1, etc.).";
  ...
  remote_ops.to_insert_fork_catchpoint = return_zero_int;
  remote_ops.to_insert_vfork_catchpoint = return_zero_int;
  remote_ops.to_insert_exec_catchpoint = return_zero_int;
  remote_ops.to_remove_fork_catchpoint = return_zero_int;
  remote_ops.to_remove_vfork_catchpoint = return_zero_int;
  remote_ops.to_remove_exec_catchpoint = return_zero_int;
...
Specify the serial device it is connected to (e.g. /dev/ttya).";
  ...
  remote_async_ops.to_follow_fork = remote_follow_fork;
  remote_async_ops.to_insert_fork_catchpoint = return_zero_int;
  remote_async_ops.to_insert_vfork_catchpoint = return_zero_int;
  remote_async_ops.to_insert_exec_catchpoint = return_zero_int;
  remote_async_ops.to_remove_fork_catchpoint = return_zero_int;
  remote_async_ops.to_remove_vfork_catchpoint = return_zero_int;
  remote_async_ops.to_remove_exec_catchpoint = return_zero_int;
...
show_remote_cmd (char *args, int from_tty)
{
  ...
  show_remote_protocol_p_packet_cmd (args, from_tty, NULL);
  show_remote_protocol_qPassSignals_packet_cmd (args, from_tty, NULL);
  show_remote_protocol_qSymbol_packet_cmd (args, from_tty, NULL);
  show_remote_protocol_vcont_packet_cmd (args, from_tty, NULL);
  show_remote_protocol_binary_download_cmd (args, from_tty, NULL);
  show_remote_protocol_qPart_auxv_packet_cmd (args, from_tty, NULL);
  show_remote_protocol_qPart_availableRegisters_packet_cmd (args, from_tty, NULL);
}
...
in a memory packet.\n",
  ...
  add_packet_config_cmd (&remote_protocol_qPassSignals,
			 "qPassSignals", "pass-signals",
			 set_remote_protocol_qPassSignals_packet_cmd,
			 show_remote_protocol_qPassSignals_packet_cmd,
			 &remote_set_cmdlist, &remote_show_cmdlist,
			 0);
