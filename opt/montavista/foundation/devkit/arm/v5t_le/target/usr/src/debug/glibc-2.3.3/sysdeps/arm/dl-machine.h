...
elf_machine_plt_value (struct link_map *map, const Elf32_Rel *reloc,
		       Elf32_Addr value)
{
  Elf32_Sym *sym = (Elf32_Sym *) map;
  if (ELF32_ST_TYPE (sym->st_info) == STT_ARM_TFUNC)
    return value | 1;
  return value;
}
...
#endif
  if (__builtin_expect (r_type == R_ARM_NONE, 0))
    return;
  else
    {
...
# endif
	value += sym->st_value;
      ...
      if (sym && ELF32_ST_TYPE (sym->st_info) == STT_ARM_TFUNC)
	value |= 1;
