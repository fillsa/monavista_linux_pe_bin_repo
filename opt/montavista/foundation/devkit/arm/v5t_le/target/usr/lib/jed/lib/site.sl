...
% If it is non-zero, the \var{'{'} character will be placed on a line by
% itself when one presses the \var{'{'} character.  For K&R indentation style,
...
%\usage{String_Type search_path_for_file (String_Type path, String_Type file)}
%\description
...
% \NULL.
%!%-
define search_path_for_file (path, f)
{
   foreach (strtok (path, ","))
     {
        variable dir = ();
...
$1 = JED_ROOT;
#else
$1 = dircat (JED_ROOT, "doc/txt");
#endif

foreach (["jedfuns.txt", "libfuns.txt"])
{
   $2 = ();
#ifdef VMS
   $2 = "[doc.txt]" + $2;
...
Jed_Doc_Files = strcat (Jed_Doc_Files, ",",
			path_concat (_slang_doc_dir, "slangfun.txt"));
#endif
__uninitialize (&$1);
__uninitialize (&$2);
...
static define make_reserved_key (key)
{
...
static define run_user_exit_hook ()
{
   runhooks ("exit_hook");
   return 1;
}
...
variable Mode_List_Exts = "h,cc,cpp,hpp,sl,txt,doc,f,for,pro,1,pl,pm,v,verilog,vhd,vhdl,vt,sp,cir,py,cxx,m,bib,sgml";
variable Mode_List_Modes = "c,c,c,c,slang,text,text,fortran,fortran,idl,nroff,perl,perl,verilog,verilog,vhdl,vhdl,vhdl,spice,spice,python,c,matlab,bibtex,sgml";
...
%\usage{Void set_buffer_no_backup ();}\description
%
%!%-
...
%\usage{Void set_buffer_no_autosave ();}\description
%
%!%-
...
static variable Email_Address = NULL;
...
{
   loop (_NARGS)
     mode_set_mode_info ((), "use_dfa_syntax", 1);
}
...
static define patch_cmdline_file (file)
{
#ifdef UNIX
   variable ch = file[0];
...
define command_line_hook () %{{{
{
   ...
   if (init_file != NULL)
	...
	% debian specific, added by cpbotha@debian.org
	% you might think you want to change this, but you don't want to
	depth = _stkdepth ();
	() = evalfile ("/etc/jed-defaults.sl");
	depth = _stkdepth () - depth;
	if (depth)
	  {
	     flush ("Excess junk left on stack by /etc/jed-init.d/*.sl");
	     usleep (1000);
	     _pop_n (depth);
	  }
     }
