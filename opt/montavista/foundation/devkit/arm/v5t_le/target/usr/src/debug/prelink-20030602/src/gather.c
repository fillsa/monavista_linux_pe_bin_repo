...
gather_deps (DSO *dso, struct prelink_entry *ent)
{
  ...
  if (prelink_rtld == NULL)
      ...
      if (ld_library_path)
	{
      ...
      if (sysroot)
	{
	  envp[2] = alloca (sizeof "PRELINK_SYSROOT=" + strlen (sysroot));
	  sprintf ((char *) envp[2], "PRELINK_SYSROOT=%s", sysroot);
	  envp[3] = NULL;
	}
      else
	envp[2] = NULL;
...
add_dir_to_dirlist (const char *name, dev_t dev, int flags)
{
  ...
  canon_name = wrap_canonicalize_file_name (name);
  if (canon_name == NULL)
    {
      if (! all && implicit)
	return 0;
...
gather_object (const char *name, int deref, int onefs)
{
  ...
  if (S_ISDIR (st.st_mode))
	...
	return add_dir_to_dirlist (name, st.st_dev, flags);
      ++implicit;
      ret = wrap_nftw64 (name, gather_func, 20, flags);
      --implicit;
      return ret;
    }
  else
