...
static int parse_argv(argv_helper *info, int argc, char **argv)
{
	...
	switch(p->type) {
		...
		*(p->u.iptr) = p->value;
		...
		l = strtol(s, &t, base);
		if (*t) {
		    fprintf(stderr, "%s: Invalid arg. \"%s\" to \"%s\"\n",
			    prgname, s, p->name);
		    errors++;
		    break;
		}
    ...
    if (errors)
	fprintf(stderr, "%s: Continuing despite errors in option parsing\n",
		prgname);
    return errors;
}
...
static int option_modem_lines(int fd, int argc, char **argv)
{
    ...
    if (argc<2) return 0;
    if (argc > 2) {
	fprintf(stderr, "%s: Too many options for \"-t %s\"\n",
		prgname, argv[0]);
...
static int M_summa(Gpm_Event *state, unsigned char *data)
{
  ...
  x = ((data[2]<<7) | data[1])-SUMMA_BORDER; if (x<0) x=0; if (x>summamaxx) x=summamaxx;
  state->x = (x * win.ws_col / summamaxx);
  realposx = (x * 16383 / summamaxx);

  y = ((data[4]<<7) | data[3])-SUMMA_BORDER; if (y<0) y=0; if (y>summamaxy) y=summamaxy;
...
static int M_wacom(Gpm_Event *state, unsigned char *data)
{
  ...
  if (!(data[0]&64)) /* Tool not in proximity or out of active area */ 
	     ...
	     if (data[0]&8) /* here: macro button has been pressed */
	       {
	         ...
	         if (data[3]&32) macro=(data[6])+24;  /* rom-version >= 1.3 */
                 ...
                 /*
	         gpm_debug_log(LOG_NOTICE, "WACOM Macro: %d \n", macro); 
    ...
    if (WacomAbsoluteWanted){                               /* Absolute Mode */
      ...
      if (
             abs(x-ox)>(wmaxx/wcmodell[WacomModell].treshold) 
...
static int M_gunze(Gpm_Event *state,  unsigned char *data)
{
	...
	gpm_debug_log(LOG_NOTICE, "gunze: invalid packet \"%s\"\n", data);
...
static Gpm_Type*
I_empty(int fd, unsigned short flags,
	 struct Gpm_Type *type, int argc, char **argv)
{
    if (check_no_argv(argc, argv)) return NULL;
    return type;
...
static Gpm_Type*
I_serial(int fd, unsigned short flags,
	 struct Gpm_Type *type, int argc, char **argv)
{
...
static Gpm_Type*
I_logi(int fd, unsigned short flags,
       struct Gpm_Type *type, int argc, char **argv)
{
  int i;
  struct stat buf;
...
#define UD_STOP          "\nSP\r"   /* stop sending coordinates             */
  ...
  */
  ...
  if (WacomModell >= (sizeof(wcmodell) / sizeof(struct WC_MODELL))) 
     WacomModell=-1;
  gpm_debug_log(LOG_NOTICE, "WACOM Mode: %c Modell: %s (Answer: %s)\n", 
                type->absolute? 'A':'R',
			  ...
			  struct Gpm_Type *type, int argc, char **argv)
{
	...
	fprintf(stderr, "%s: %s: wrong baud rate, using 19200\n",
		prgname, argv[0]);
	opt_baud = 19200;
    }
    tcgetattr(fd, &tty);
    ...
    if (!calibok) {
	fprintf(stderr,	"%s: gunze: calibration data absent or invalid, "
		"using defaults\n", prgname);
