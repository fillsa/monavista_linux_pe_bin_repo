...
/* ``all_threads'' is keyed by the LWP ID (process ID) while we only have one thread,
   but as soon as libthread_db takes over we switch to indexing by thread ID.

   ``all_processes'' is keyed by the process ID - which on Linux is (presently)
   the same as the LWP ID.  These are the actual waitpid-able processes we are
   managing.  */
...
unsigned long linux_memory_pid = 0;
...
static void linux_detach_one_process (struct inferior_list_entry *entry);
...
#define PTRACE_ARG3_TYPE long long
...
/*****/
/* Extended wait status support.

   Some versions of the Linux kernel support returning event codes at
   particular event points, including fork and vfork.  We report these
   events to the GDB client.  */
...
static int
handle_extended_wait (int wstat)
{
  int event = wstat >> 16;

  if (event == PTRACE_EVENT_FORK || event == PTRACE_EVENT_VFORK)
    {
      PTRACE_XFER_TYPE new_pid;
      int ret, status;
      ...
      if (! pull_pid_from_list (&stopped_pids, new_pid))
	{
      ...
      pending_fork_thread = ((struct inferior_list_entry *)current_inferior)->id;
      pending_fork_pid = new_pid;
      pending_fork_vfork = (event == PTRACE_EVENT_VFORK);
      return FORK_STATUS;
    }
  ...
  if (event == PTRACE_EVENT_EXEC)
    {
      ...
      sprintf (procpath, "/proc/%ld/exe", ((struct inferior_list_entry *)current_inferior)->id);
      readlink (procpath, pending_exec_name, 256);
      /* FIXME: basically assumes we're down to one thread by this point.
         We will be in LinuxThreads.  */
      return EXEC_STATUS;
    }
  
  /* FIXME: At an exec event it would be nice to be able to detach breakpoints
     in the parent half of a vfork and only then detach it.  We don't have
     the machinery in the remote server to do the detach.  */
  /* FIXME: Actually, we just barely do, now.  */

  return wstat;
}

static void
linux_follow (unsigned long is_child)
{
  unsigned long child_pid = pending_fork_pid;
  unsigned long parent_pid;

  current_inferior = (struct thread_info *) find_inferior_id (&all_threads,
							      pending_fork_thread);
  if (!current_inferior)
    error ("Thread vanished!");
  parent_pid = pid_of (get_thread_process (current_inferior));

  /* Disable breakpoints in whichever we will be releasing.  */
  linux_memory_pid = is_child ? parent_pid : child_pid;
  disable_all_breakpoints ();

  if (! is_child)
    {
      if (! pending_fork_vfork)
	ptrace (PTRACE_DETACH, child_pid, 0, 0);
      else
	{
	  /* We can't insert breakpoints until the child has finished with
	     the shared memory region.  We need to wait until that happens.
	     Ideal would be to just call:
	      - ptrace (PTRACE_SYSCALL, parent_pid, 0, 0);
	      - waitpid (parent_pid, &status, __WALL);
	     However, most architectures can't handle a syscall being traced
	     on the way out if it wasn't traced on the way in.  Unless this is
	     fixed we must improvise.

	     We might also think to loop, continuing the child until
	     it exits or gets a SIGTRAP.  One problem is that the
	     child might call ptrace with PTRACE_TRACEME.

	     There's no simple and reliable way to figure out when the
	     vforked child will be done with its copy of the shared
	     memory.  We could step it out of the syscall, two instructions,
	     let it go, and then single-step the parent once.  When we have
	     hardware single-step, this would work; with software single-step
	     it could still be made to work but we'd have to be able to insert
	     single-step breakpoints in the child, and we'd have to insert
	     -just- the single-step breakpoint in the parent.  Very awkward.

	     In the end, the best we can do is to make sure it runs
	     for a little while.  Hopefully it will be out of range
	     of any breakpoints we reinsert.  Usually this is only
	     the single-step breakpoint at vfork's return point.  */
  ...
  else
    {
      ...
      /* It would be nice to hold on to the parent if we're vforking, but we don't
	 have any way to detach the breakpoints from it.  Oooops.  Just detach.  */

      for_each_inferior (&all_threads, linux_detach_one_process);
      first_wait = 1;
      clear_inferiors ();
      while (all_processes.head != NULL)
	{
	  struct process_info *p;
	  p = (struct process_info *) all_processes.head;

	  remove_inferior (&all_processes, all_processes.head);
	  free (p);
	}
      new_process = (struct process_info *) add_process (child_pid);
      new_process->stopped = 1;
      add_thread (child_pid, new_process, child_pid);
      /* We've created the process with its thread_known flag cleared, so
	 we will fill in its TID next time we enter thread_db.  */
      current_inferior = (struct thread_info *) all_threads.head;
    }

  /* Enable breakpoints in whichever we held on to, if the memory
     space was shared, causing us to remove them above.  */
  if (pending_fork_vfork)
    {
      linux_memory_pid = is_child ? child_pid : parent_pid;
...
check_removed_breakpoint (struct process_info *event_child)
{
  ...
  if (stop_pc != event_child->pending_stop_pc)
    {
      if (debug_threads)
	fprintf (stderr, "Ignoring, PC was changed.  Old PC was %08llx.\n",
	  ...
	  if (WIFSTOPPED (wstat)
	      && ((WSTOPSIG (wstat) == __SIGRTMIN
		   ...
		   || (pass_signals[target_signal_from_host (WSTOPSIG (wstat))]
		       && (WSTOPSIG (wstat) != SIGSTOP || !event_child->sigstop_sent))))
	    {
	      if (debug_threads)
...
retry:
  ...
  if (first_wait)
    {
      ptrace (PTRACE_SETOPTIONS, inferior_pid, 0,
	      PTRACE_O_TRACEFORK | PTRACE_O_TRACEVFORK | PTRACE_O_TRACEEXEC);
      first_wait = 0;
    }

  if (WIFSTOPPED (w) && WSTOPSIG (w) == SIGTRAP && w >> 16 != 0)
    {
      *status = 'T';
      return handle_extended_wait (w);
    }
...
linux_fetch_registers (int regno)
{
  ...
  if (use_regsets_p)
    {
#ifdef LINUX_USE_BOTH_REGISTERS
      regsets_fetch_inferior_registers ();
#else
...
linux_store_registers (int regno)
{
  ...
  if (use_regsets_p)
    {
#ifdef HAVE_LINUX_USE_BOTH_REGISTERS
      regsets_store_inferior_registers ();
#else
...
linux_read_memory (CORE_ADDR memaddr, char *myaddr, int len)
{
  ...
  unsigned long pid = inferior_pid;
  if (linux_memory_pid)
    pid = linux_memory_pid;
