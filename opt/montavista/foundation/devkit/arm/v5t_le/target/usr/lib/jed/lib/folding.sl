...
static define fold_is_marker_line (start, end_of_start) %{{{
{
   bol_skip_white ();
   if (looking_at (start))
     {
...
%}}}
static define fold_find_marker_line (start, end_of_start) %{{{
{
   bol ();
   while (fsearch (start))
     {
...
%}}}
static define fold_find_marker_line_reverse (start, end_of_start, hidden_check) %{{{
{
   eol ();
   while (bsearch (start))
     {
...
%}}}
static define fold_this_fold (start, end, end_of_start, end_of_end, 
...
%}}}
static define fold_is_fold (start, end_of_start) %{{{
{
   push_spot ();
   EXIT_BLOCK
     {
...
static define fold_exit_fold_internal () %{{{
{
   !if (count_narrows ())
     {
